...
{
    private enum Kind : byte { None, Number, Text }
...
public static class Columns
{
    ...
    public static string Title(Column c) => c switch
    {
        Column.Name => "Process",
        Column.Pid => "PID",
    ...
    public static double DefaultWidth(Column c) => c switch
    {
        ...
        Column.PrivateBytes or Column.WorkingSet or Column.VirtualSize
            or Column.SharedBytes or Column.SwapBytes => 96,
        Column.Threads => 62,
        Column.Handles => 68,
    ...
    public static bool IsRightAligned(Column c) => c switch
    {
        Column.Name or Column.Description or Column.Company or Column.Version or
        Column.Path or Column.CommandLine or Column.User or Column.StartTime or
        Column.Provenance or Column.Autostart or Column.Integrity or
        Column.CgroupPath or Column.SystemdUnit or Column.SecurityLabel or
        Column.State or Column.SchedulingPolicy => false,
        _ => true,
    };
    ...
    [
        Column.Name, Column.Pid, Column.Cpu, Column.PrivateBytes,
        Column.WorkingSet, Column.Description, Column.Company, Column.Provenance,
    ];

    /// <summary>The display string for a column and process.</summary>
    public static string Format(Column c, ProcessRecord p) => c switch
    {
        Column.Name => p.Name,
        Column.Pid => p.Id.Pid.ToString(CultureInfo.InvariantCulture),
        Column.Ppid => p.Parent is { } parent ? parent.Pid.ToString(CultureInfo.InvariantCulture) : "",
        Column.Cpu => p.CpuPercent < 0.01 ? "" : p.CpuPercent.ToString("F2", CultureInfo.InvariantCulture),
        Column.CpuTime => ValueFormat.Duration(p.CpuTime),
        Column.PrivateBytes => ValueFormat.Bytes(p.ProportionalSetSize),
        ...
        Column.Gpu => p.GpuPercent is { } g ? g.ToString("F1", CultureInfo.InvariantCulture) : "",
        ...
        Column.VirusTotal => p.Provenance?.VirusTotal is { } vt ? $"{vt.Positives}/{vt.Total}" : "",
        Column.Autostart => p.AutostartLocation ?? "",
        _ => "",
    };

    /// <summary>A comparable sort key for a column and process.</summary>
    public static SortKey SortValue(Column c, ProcessRecord p) => c switch
    {
        ...
        Column.PrivateBytes => Nullable(p.ProportionalSetSize),
        ...
        Column.VirusTotal => p.Provenance?.VirusTotal is { } vt ? SortKey.Number(vt.Positives) : SortKey.None,
...
public static class ValueFormat
{
    ...
    public static string Integer(ulong? value) => value?.ToString(CultureInfo.InvariantCulture) ?? "";

    /// <summary>Decode the <c>/proc/PID/stat</c> state character.</summary>
    public static string ProcessState(char state) => state switch
    {
        'R' => "Running",
        'S' => "Sleeping",
        'D' => "Disk Sleep",
    ...
    public static string SchedulingPolicy(int? policy) => policy switch
    {
        0 => "Normal",
        1 => "FIFO",
        2 => "RR",
