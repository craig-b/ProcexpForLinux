...
{
    Console.WriteLine(
        $"  [{(ok ? "PASS" : "FAIL")}] {what}{(detail is null ? "" : $" — {detail}")}"
    );
    if (!ok)
    {
        failures++;
...
Check(
    "thread count is plausible",
    snapshot.System.ThreadCount >= snapshot.Processes.Count,
...
Check(
    "own start time is sane",
    self is not null
        && self.StartTime > DateTimeOffset.Now.AddHours(-1)
        && self.StartTime <= DateTimeOffset.Now,
...
var hasKernelThreads = snapshot.Processes.Values.Any(p =>
    p.Flags.HasFlag(ProcessFlags.KernelThread)
);

Console.WriteLine();
Console.WriteLine(
    $"Environment: {(isContainerLike ? "container-like" : "full system")}, "
...
var psCount = RunCommand("ps", "-e --no-headers")
...
if (psCount is { } expected)
{
    ...
    Check(
        "process count matches ps",
        delta <= Math.Max(25, expected / 20),
...
if (int.TryParse(psPpid, out var expectedPpid) && self is not null)
{
    ...
    Check(
        "parent pid matches ps",
        ourPpid == expectedPpid || (ourPpid is null && expectedPpid == 0),
...
CheckWhere(
    !isContainerLike,
    "too few processes for anything to be measurably busy",
    "second frame has CPU rates",
    busy.Count > 0,
    $"{busy.Count} processes with measurable CPU"
);

var cores = Environment.ProcessorCount;
var overrun = frame2.Processes.Values.Where(p => p.CpuPercent > cores * 100.0 + 50).ToList();
Check(
    "no process exceeds total machine capacity",
    overrun.Count == 0,
...
Check(
    "threads carry real detail, not stubs",
...
Check(
    "module list matches /proc/self/maps",
    Math.Abs(modules.Count - distinctMapped) <= 2,
...
if (init is not null && !runningAsRoot)
{
    ...
    Check(
        "other-user process has a readable command line",
        !string.IsNullOrEmpty(init.CommandLine),
...
CheckWhere(
    hasSystemd,
    "no systemd on this system",
    "services name their unit",
    services.Any(p => p.SystemdUnit is not null),
    services.FirstOrDefault(p => p.SystemdUnit is not null)?.SystemdUnit
);

var kernelThreads = snapshot
    .Processes.Values.Where(p => p.Flags.HasFlag(ProcessFlags.KernelThread))
    .ToList();
CheckWhere(
    !isContainerLike,
    "a pid namespace hides kernel threads",
    "kernel threads detected",
    kernelThreads.Count > 0,
    $"{kernelThreads.Count} kernel threads"
);
CheckWhere(
    hasKernelThreads,
    "none visible here",
    "kernel threads have no image",
    kernelThreads.All(p => p.ExecutablePath is null)
);
...
Check(
    ...
    provenance.PackageManager != PackageManagerKind.None,
...
var lsPath =
    new[] { "/usr/bin/ls", "/bin/ls", "/usr/bin/coreutils" }.FirstOrDefault(File.Exists)
    ?? "/usr/bin/ls";
if (File.Exists(lsPath))
{
    var ls = await provenance.ProvenanceAsync(lsPath);
    Check(
        "distribution binary is attributed to a package",
        ls.Status == ProvenanceStatus.PackageVerified,
        ls.DisplayName
    );
    ...
    var owner =
        RunCommand("pacman", $"-Qoq {lsPath}")?.Trim()
    ...
    if (!string.IsNullOrEmpty(owner))
    {
        ...
        Check(
            "package name matches the native tool",
            ls.PackageName is { } ours
                && (ours == owner || ours.StartsWith(owner, StringComparison.Ordinal)),
...
if (ownImage is not null)
{
    ...
    Check(
        "unpackaged binary is reported as unpackaged",
        own.Status == ProvenanceStatus.Unpackaged,
        own.DisplayName
    );
}
...
if (readelf is not null && readelf.Contains("Build ID", StringComparison.Ordinal))
{
    ...
    Check(
        "build-id matches readelf",
        elf.BuildId == expectedBuildId,
...
foreach (
    var candidate in snapshot.Processes.Values.Where(p =>
        !p.Flags.HasFlag(ProcessFlags.KernelThread)
    )
)
{
...
CheckWhere(
    !isContainerLike,
    "nothing here holds sockets",
    "sockets found across processes",
    withSockets.Count > 0,
...
CheckWhere(
    tcp.Count > 0,
    "no TCP sockets in this environment",
    "TCP sockets present",
    tcp.Count > 0,
    $"{tcp.Count} TCP sockets"
);
CheckWhere(
    tcp.Count > 0,
    "no TCP sockets in this environment",
    "TCP states decode",
    tcp.All(s => s.State.Length > 0) && tcp.Any(s => s.State is "ESTABLISHED" or "LISTEN")
);
Check(
    "listening sockets have a local port",
    tcp.Where(s => s.State == "LISTEN").All(s => s.LocalPort > 0)
);
Check(
    "addresses parse as valid IPs",
...
CheckWhere(
    !isContainerLike,
    "nothing here holds unix sockets",
    "unix sockets present",
    unix.Count > 0,
    $"{unix.Count} unix sockets"
);
...
{
    var ssPorts = ssOutput
        .Split('\n', StringSplitOptions.RemoveEmptyEntries)
        .Skip(1)
        .Select(l => l.Split(' ', StringSplitOptions.RemoveEmptyEntries))
        .Where(f => f.Length > 3)
        .Select(f =>
            f[3].LastIndexOf(':') is var i && i >= 0 && ushort.TryParse(f[3][(i + 1)..], out var p)
                ? p
                : (ushort)0
        )
        .Where(p => p > 0)
    ...
    Check(
        "listening ports are a subset of ss",
        missing.Count < ssPorts.Count || ssPorts.Count == 0,
...
Check(
    "per-process rates are empty by design",
    rates.Count == 0,
...
Check(
    "memory total is plausible",
    stats.MemoryTotal > 1024UL * 1024 * 256,
    ValueFormat.Bytes(stats.MemoryTotal)
);
Check(
    "memory used is below total",
    stats.MemoryUsed > 0 && stats.MemoryUsed < stats.MemoryTotal,
    $"{ValueFormat.Bytes(stats.MemoryUsed)} used"
);
Check(
    "per-core CPU matches core count",
    stats.PerCoreCpuPercent.Count == Environment.ProcessorCount,
    $"{stats.PerCoreCpuPercent.Count} cores reported, {Environment.ProcessorCount} expected"
);
Check(
    "CPU percentages are in range",
    stats.CpuTotalPercent is >= 0 and <= 100
        && stats.PerCoreCpuPercent.All(c => c is >= 0 and <= 100),
...
var memTotalKb = File.ReadLines("/proc/meminfo")
    .FirstOrDefault(l => l.StartsWith("MemTotal:", StringComparison.Ordinal));
if (memTotalKb is not null)
{
    var expectedBytes =
        ulong.Parse(memTotalKb.Split(':')[1].Trim().Split(' ')[0], CultureInfo.InvariantCulture)
        * 1024;
...
Check(
    "CPU model identified",
    hardware.CpuModel.Length > 0 && hardware.CpuModel != "Unknown CPU",
    hardware.CpuModel
);
Check(
    "logical cores match runtime",
    hardware.LogicalCores == Environment.ProcessorCount,
...
Check(
    "cache sizes read",
    hardware.L1DataCache is > 0 && hardware.L3Cache is > 0,
...
Check(
    ...
    hardware.DistributionName is { Length: > 0 },
    hardware.DistributionName
);
...
CheckWhere(
    !isContainerLike,
    "container root is an overlay, not a real volume",
    "boot volume present",
    hardware.Volumes.Any(v => v.IsBootVolume),
    $"{hardware.Volumes.Count} real volumes"
);
Check(
    "no pseudo filesystems listed",
    hardware.Volumes.All(v => v.FileSystem is not ("tmpfs" or "proc" or "sysfs" or "cgroup2"))
);
Check(
    "network interfaces enumerated",
    hardware.NetworkInterfaces.Count > 0,
...
{
    Check(
        "core count matches nproc",
        hardware.LogicalCores == expectedCores,
        $"nproc says {expectedCores}"
    );
}
...
CheckWhere(
    ...
    videoMemory is null
        ? null
        : $"{ValueFormat.Bytes(videoMemory.Value.Used)} / {ValueFormat.Bytes(videoMemory.Value.Total)}"
);
...
Check(
    "no process reports impossible GPU usage",
...
CheckWhere(
    hasSystemd,
    "nothing defines autostart without an init system",
    "autostart index built",
    autostart.Index.Count > 0,
    $"{autostart.Index.Count} definitions"
);
...
CheckWhere(
    hasSystemd && !isContainerLike,
    "no init-managed processes here",
    "running processes resolve to autostart definitions",
    resolved.Count > 0,
    $"{resolved.Count} of {snapshot.Processes.Count} processes"
);
CheckWhere(
    hasSystemd && !isContainerLike,
    "no systemd units running here",
    "systemd services resolve",
    resolved.Any(r => r.Location.StartsWith("systemd", StringComparison.Ordinal))
);
Check(
    "kernel threads never resolve",
    (
        await Task.WhenAll(
            snapshot
                .Processes.Values.Where(p => p.Flags.HasFlag(ProcessFlags.KernelThread))
...
using (
    var victim = Process.Start(
        new ProcessStartInfo("sleep") { ArgumentList = { "300" }, UseShellExecute = false }
    )
)
{
    if (victim is null)
    ...
    else
    {
        ...
        var victimRecord = victimSnapshot.Processes.Values.FirstOrDefault(p =>
            p.Id.Pid == victim.Id
        );
        ...
        if (victimRecord is not null)
        {
            ...
            Check(
                "suspend sets the stopped state",
                suspended?.State is 'T' or 't',
            ...
            Check(
                "resume clears the stopped state",
                resumed?.State is not ('T' or 't'),
                $"state {resumed?.State}"
            );
            ...
            Check(
                "renice takes effect",
                afterNice.Processes.GetValueOrDefault(victimId)?.Nice == 5,
            ...
            var recycled = victimId with
            {
                StartTime = victimId.StartTime + 12345,
            };
            var guarded = false;
            try
            {
            ...
            Check("stale identity is refused rather than signalled", guarded, "PID-reuse guard");
...
if (init is not null)
{
    ...
    Check("killing pid 1 is refused by policy", confirmation.IsRefused, confirmation.Message);
...
var service = snapshot.Processes.Values.FirstOrDefault(p =>
    p.Flags.HasFlag(ProcessFlags.Service) && p.Id.Pid != 1
);
if (service is not null)
{
    var confirmation = ActionConfirmationPolicy.ForKill(service);
    Check(
        "services warn about systemd restart",
...
Measure(
    "without fd counting",
    ProcSamplerOptions.Default with
    {
        IncludeFileDescriptorCount = false,
    }
);
Measure(
    "with PSS (smaps_rollup)",
    ProcSamplerOptions.Default with
    {
